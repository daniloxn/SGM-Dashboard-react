  ...
  return (
    <div ref={ref} className={clsx('relative', className)}>
      ...
      <button
        type="button"
        onClick={() => setOpen(v => !v)}
        className="flex items-center justify-between w-full px-3 py-2 bg-slate-900/60 border border-white/10 rounded-lg text-sm text-slate-300 hover:border-blue-500/40 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all gap-2"
      >
        <span className="truncate text-left">{headerLabel}</span>
        <svg className={clsx('w-4 h-4 text-slate-400 shrink-0 transition-transform', open && 'rotate-180')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute z-50 mt-1 w-full min-w-[220px] bg-slate-800 border border-white/10 rounded-xl shadow-2xl overflow-hidden">
          {/* Toolbar */}
          <div className="p-2 border-b border-white/5 space-y-2">
            <input
              ...
              className="w-full bg-slate-900/60 border border-white/10 rounded-lg px-3 py-1.5 text-xs text-slate-300 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition-all"
            />
            <div className="flex items-center justify-between px-0.5">
              <div className="flex gap-2">
                <button onClick={selectAll} type="button" className="text-xs text-blue-400 hover:text-blue-300 transition-colors">Marcar Todos</button>
                <span className="text-slate-600">|</span>
                <button onClick={clearAll} type="button" className="text-xs text-slate-400 hover:text-slate-300 transition-colors">Limpar</button>
              </div>
          ...
          <div className="max-h-60 overflow-y-auto">
            {/* Default "all" option */}
            <label className="flex items-center gap-2.5 px-3 py-2 hover:bg-slate-700/50 cursor-pointer border-b border-white/5">
              <input
                type="checkbox"
                className="rounded border-slate-600 bg-slate-900 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-800"
                checked={isAll}
                onChange={() => clearAll()}
              />
              <span className="text-sm font-semibold text-blue-400">{defaultLabel}</span>
            </label>
            ...
            {filtered.map(opt => (
              <label key={opt.value} className="flex items-center gap-2.5 px-3 py-2 hover:bg-slate-700/50 cursor-pointer">
                <input
                  type="checkbox"
                  className="rounded border-slate-600 bg-slate-900 text-blue-500 focus:ring-blue-500 focus:ring-offset-slate-800"
