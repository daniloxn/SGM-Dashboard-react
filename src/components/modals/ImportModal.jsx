    ...
    <Modal open={open} onClose={handleClose} title="📥 Importar Planilha de Manutenção" size="md">
      {step === 'preview' && !preview && (
        <div className="space-y-5">
          ...
          <p className="text-slate-400 text-sm">Selecione o arquivo Excel (.xlsx, .xls) com as Ordens de Serviço.</p>
          <label className="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-blue-500/30 rounded-xl hover:border-blue-500/60 hover:bg-blue-500/5 cursor-pointer transition-all">
            <span className="text-3xl mb-2">📂</span>
            <span className="text-sm font-semibold text-blue-400">Clique para selecionar o arquivo</span>
            <span className="text-xs text-slate-500 mt-1">Formatos suportados: .xlsx, .xls</span>
            <input type="file" accept=".xlsx,.xls" className="hidden" onChange={handleFile} />
          </label>
          {error && <div className="text-red-400 text-sm bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-3">⚠️ {error}</div>}
        </div>
      )}
      ...
      {step === 'preview' && preview && (
        <div className="space-y-4">
          {/* Summary Box */}
          <div className="bg-slate-900/80 rounded-xl p-4 border border-white/5 space-y-3">
            <div className="flex items-center justify-between border-b border-white/5 pb-2">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Total de O.S.</span>
              <span className="text-lg font-bold text-white">{preview.dados.length}</span>
            </div>
            ...
            <div className="space-y-1 pt-1">
              <span className="text-xs text-slate-400">Distribuição por Mês:</span>
              <div className="flex flex-wrap gap-1.5 mt-1">
                {preview.meses.map(m => (
                  <span key={m} className="text-xs bg-slate-800 text-slate-200 px-2.5 py-1 rounded-md border border-white/10">
          ...
          {/* Strategy Selection */}
          <div className="bg-slate-900/50 rounded-xl p-3.5 border border-white/5 space-y-2">
            <p className="text-xs font-semibold text-slate-300">Modo de Importação:</p>
            
            <label className={`flex items-start gap-3 p-2.5 rounded-lg cursor-pointer border transition-all ${modo === 'upsert' ? 'bg-blue-500/10 border-blue-500/40 text-white' : 'border-white/5 text-slate-400 hover:bg-slate-800/50'}`}>
              <input
                type="radio"
                name="modoImport"
                value="upsert"
              ...
              <div className="text-xs leading-relaxed">
                <span className="font-semibold block text-slate-200">📅 Importação Diária / Incremental (Recomendado)</span>
            ...
            <label className={`flex items-start gap-3 p-2.5 rounded-lg cursor-pointer border transition-all ${modo === 'substituir' ? 'bg-amber-500/10 border-amber-500/40 text-white' : 'border-white/5 text-slate-400 hover:bg-slate-800/50'}`}>
              <input
                type="radio"
                name="modoImport"
                value="substituir"
              ...
              />
              <div className="text-xs leading-relaxed">
                <span className="font-semibold block text-slate-200">⚠️ Substituir Meses Inteiros</span>
      ...
      {step === 'loading' && (
        <div className="space-y-5 py-4">
          <div className="text-center">
            ...
            <p className="text-slate-300 font-medium">Gravando na nuvem...</p>
      ...
      {step === 'done' && (
        <div className="text-center py-6">
          ...
          <p className="text-slate-400 text-sm mt-1">Os dados foram sincronizados com sucesso.</p>
        </div>
      )}
    </Modal>
  );
