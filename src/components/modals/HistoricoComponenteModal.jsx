...
export default function HistoricoComponenteModal({ open, onClose, componente, onSuccess }) {
  ...
  const badgeLocalizacao = {
    OFICINA_MANUTENCAO: { text: '🛠️ Na Oficina (Em Manutenção)', bg: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
    OFICINA_RESERVA: { text: '📦 Na Oficina (Pronta / Reserva)', bg: 'bg-blue-500/20 text-blue-300 border-blue-500/30' },
    SONDA: { text: `🏗️ Na Sonda ${componente.sondaAtual || '-'}`, bg: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' }
  }[componente.localizacao] || { text: componente.localizacao || 'Desconhecida', bg: 'bg-slate-700 text-slate-300 border-slate-600' };
    ...
    >
      <div className="space-y-4">
        ...
        <div className="bg-slate-900 border border-white/5 rounded-xl p-3 flex items-center justify-between">
          <div>
            <span className="text-[10px] text-slate-400 block font-semibold uppercase">Localização Atual</span>
          ...
          <button
            ...
            className="text-xs px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-white/10 transition-colors font-medium"
          >
        ...
        {modoAjuste && (
          <div className="bg-slate-900/90 border border-blue-500/30 rounded-xl p-3.5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">Nova Localização</label>
                <select
                  value={novaLocalizacao}
                  onChange={e => setNovaLocalizacao(e.target.value)}
                  className="w-full bg-slate-800 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
              ...
              {novaLocalizacao === 'SONDA' && (
                <div>
                  <label className="block text-[11px] text-slate-400 mb-1">Sonda Alocada</label>
                  <input
                    type="text"
              ...
              <div className="col-span-1 sm:col-span-2">
                <label className="block text-[11px] text-slate-400 mb-1">Motivo do Ajuste</label>
                <input
                  type="text"
            ...
            {componente.historico && componente.historico.length > 0 ? (
              componente.historico.map((ev, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-slate-900/60 border border-white/5 rounded-xl p-2.5 text-xs space-y-1"
                  >
                    ...
                    {ev.observacao && (
                      <p className="text-slate-300 text-[11px]">{ev.observacao}</p>
                    )}
                  </div>
                );
              })
